...
def child(task_dir: Path) -> None:
    ...
    try:
        ...
        with extraction_assets(task_dir / "figures") as notices:
            ...
            incomplete = sum(f.analysis_status not in ("complete", "source_extracted", "visual_interpretation") for f in figures)
            if incomplete:
                result.warnings.append(f"{incomplete} stored figure(s) have no completed visual analysis; available source text and OCR remain searchable.")
