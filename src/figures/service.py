...
def visual_question(question):
    return bool(re.search(r"\b(diagrams?|topolog(?:y|ies)|venn|flowcharts?|illustrations?|figures?|schematics?)\b", question, re.I))
...
async def search_chunks(query, groups, vector_store, metadata_store, top_k=5, doc_id=None, kind=None, allowed_doc_ids=None, dataset_id=0):
    ...
    scope = await resolve_query_scope(groups, metadata_store, dataset_id=dataset_id, allowed_doc_ids=allowed_doc_ids)
    ...
    chunks = await asyncio.to_thread(vector_store.search_figures, vector, query, groups, ids, top_k, kind)
