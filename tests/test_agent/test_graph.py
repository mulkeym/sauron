...
@pytest.mark.asyncio
async def test_run_agent_lookup(monkeypatch):
    from src.retrieval.query_scope import QueryScope
    monkeypatch.setattr("src.retrieval.query_scope.resolve_query_scope", AsyncMock(return_value=QueryScope(("d1",), "test")))
    mock_store = MagicMock()
